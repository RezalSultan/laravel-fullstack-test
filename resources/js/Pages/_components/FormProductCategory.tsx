...
const FormProductCategory: React.FC<FormProps> = ({
    onClose,
    productId,
    dataProduct,
    dataProductCategory,
    ...
    const { data, setData, post, processing, errors, reset } =
        useForm<FormProductCategoryRequest>({
            product_id: productId,
            desc: dataProductCategory?.desc ?? "",
            image: dataProductCategory?.image ?? null,
        });
    ...
    const onSubmit = (e: React.FormEvent) => {
        ...
        dataProductCategory
            ? post(
                  route(
                      "edit.product-category.request",
                      dataProductCategory.id
                  ),
                  {
                      onError: (errors) => {
            ...
            : post(route("create.product-category.request"), {
                  onError: (errors) => {
                      console.log("Error:", errors);
                  },
                  onSuccess: () => {
                      onClose();
                      dataProduct &&
                          dataProduct.product_categories.length == 2 &&
                          toast({
                              title: "Anda Sudah Mencapai Maksimum Input",
                              description: `Produk ${dataProduct.name} mencapai maksimum input`,
                          });
                  },
              });
    };
                ...
                >
                    ...
                    <div className="mt-4 flex w-full justify-end gap-4">
                        ...
                        <Button disabled={processing} type="submit">
                            {dataProductCategory ? "Edit" : "Tambah"}
                        </Button>
                    </div>
                </form>
            </div>
